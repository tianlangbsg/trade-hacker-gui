...
# 查询指定日期全部股票
def getStocksByDate(date):
    # 定义要执行的SQL语句
    sql = "SELECT DISTINCT stock_code FROM alternative_stock_pool WHERE DATE_FORMAT(timestamp, '%Y%m%d')='"+date+"' AND DATE_FORMAT(timestamp, '%H:%i:%S')<'14:57:00';"
    # 取到查询结果
    result = mysqlUtil.query(sql)
    return result
