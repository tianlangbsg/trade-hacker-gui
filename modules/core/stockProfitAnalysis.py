...
def calcProfit(historyStart, startDate, endDate):
    ...
    for tradeDate in tradeDateList:
        ...
        for stockCode in allStockHistoryDict.keys():
            if allStockHistoryDict[stockCode].__contains__(endDate):
                ...
                # 实际手数个股利润
                open_profit_single = (allStockHistoryDict[stockCode][endDate]['open']-allStockHistoryDict[stockCode][startDate]['high'])*float(tradeRecordDict[stockCode]['trade_amount'])
                high_profit_single = (allStockHistoryDict[stockCode][endDate]['high']-allStockHistoryDict[stockCode][startDate]['high'])*float(tradeRecordDict[stockCode]['trade_amount'])
                low_profit_single = (allStockHistoryDict[stockCode][endDate]['low']-allStockHistoryDict[stockCode][startDate]['high'])*float(tradeRecordDict[stockCode]['trade_amount'])
                close_profit_single = (allStockHistoryDict[stockCode][endDate]['close']-allStockHistoryDict[stockCode][startDate]['high'])*float(tradeRecordDict[stockCode]['trade_amount'])
    ...
    filePath = get_home_path() + "\\data\\analysis\\涨停利润趋势分析"+startDate+".html";
...
if __name__ == '__main__':
    ...
    # 交易开始日期
    startDateList = [
                     "20230419", "20230420", "20230421", "20230424", "20230425", "20230426", "20230427",
                      "20230511","20230515","20230516","20230517",
                     "20230523","20230524"
                     ]
    # 结束日期
    endDate = "20230529"
