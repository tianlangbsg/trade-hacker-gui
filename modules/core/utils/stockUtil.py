...
from modules.core.utils.commonUtil import get_home_path
...
def save_history_60(jsonObject):
    path = get_home_path() + '\\data\\history\\60\\' + datetime.datetime.now().strftime('%Y%m%d' + '.json')
...
def save_history_1(jsonObject, date=datetime.datetime.now().strftime('%Y%m%d' + '.json')):
    path = get_home_path() + '\\data\\history\\1\\' + date + '.json'
    save_json_file(jsonObject, path)
